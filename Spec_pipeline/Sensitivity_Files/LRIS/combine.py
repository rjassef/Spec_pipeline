...
if len(sys.argv)<=4:
    print("Correct use: python",sys.argv[0],"outputname.txt polyfit_degree lan_norm sens1 [sens2 [...sensN]]")
    sys.exit()
...
if len(sens)>1:
    ...
    fit_sens_func = np.poly1d(p)
    fit_sens = fit_sens_func(wave.value)
    ...
    wave = sens[0].dispersion
    fit_sens = sens[0].data
...
#for s in sens_resampled:
#    plt.plot(wave,s,'--')
plt.plot(wave,fit_sens,'b-')
#plt.show()
plt.savefig(fname+".png")

#Save the final curve.
np.savetxt(fname+".txt",np.array([wave.value, fit_sens*0.01]).T)
