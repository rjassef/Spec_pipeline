...
cato.write("{0:s} {1:s} {2:s} {3:s} ".format(
        "obj_id","dv", "flam_line", "FWHM_v"))
if estimate_errors is True:
    cato.write("{0:s} {1:s} ".format("dv_low", "dv_hig"))
    cato.write("{0:s} {1:s} ".format("flam_line_low",
                                     "flam_line_hig"))
...
for line in cat:
    ...
    cato.write("{0:s} {1:.3f} {2:.3e} {3:.3f} ".format(
        spec.name, civ_fit.dv_fit[0].value, 
        civ_fit.flam_line_fit[0].value, 
        civ_fit.FWHM_v[0].value))
    ...
    if estimate_errors is True:
        ...
        cato.write("{0:.3f} {1:.3f} ".format(civ_fit.dv_low[0].value,
                                             civ_fit.dv_hig[0].value))
        cato.write("{0:.3e} {1:.3e} ".format(civ_fit.flam_line_low[0].value,
                                             civ_fit.flam_line_hig[0].value))
        cato.write("{0:.3f} {1:.3f} ".format(civ_fit.FWHM_v_low[0].value,
                                             civ_fit.FWHM_v_hig[0].value))
