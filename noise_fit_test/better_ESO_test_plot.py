...
dlam_targ = 20. #lam_targ +/- dlam_tagr will be used to compare
                #errors.
    ...
    for i,moon_template in enumerate(moons):
        ...
        for m,moon in enumerate(moons):
            ...
            for z in (z_values):
                ...
                lam = data[:nlam]
                k = np.argwhere(np.abs(lam-lam_targ)<dlam_targ)

                #We tile k into an nrep matrix, and the add nlam*nrep
                #in every row.
                kuse = np.tile(k,nrep)
                kuse+= nlam*np.tile(np.arange(1,nrep+1),[len(k),1])
                flam_err = data[kuse]

                frac_flam_err.append(np.median(
                    (flam_err/fake_spec.flam_err[k].value),axis=0)) 
...
#plt.show()
plt.savefig("better_ESO_test.g{0:.1f}.png".format(gmag),dpi=300)
