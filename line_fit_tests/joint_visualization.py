...
em_lines = ["LyA_NV", "OII", "OIII"]

cato = open("joint_visualization.summary.txt","w")
...
for line in cat:
    ...
    for k,em_line in enumerate(em_lines):
        ...
        if 1.-line_fit.p > 0.5:
            cato.write("{0:10.1f}".format(line_fit.dv1_fit.value))
            cato.write("{0:10.1f}".format(line_fit.FWHM_v1.value))
            cato.write("{0:10.1f}".format(line_fit.dv2_fit.value))
            cato.write("{0:10.1f}".format(line_fit.FWHM_v2.value))

            zline1 = spec.zspec+line_fit.dv1_fit/c
            zline2 = spec.zspec+line_fit.dv2_fit/c
            cato.write("{0:8.3f} {1:8.3f} {2:8.3f}".format(
                spec.zspec,zline1, zline2))
        else:
            cato.write("{0:^10s} {0:^10s} {0:^10s}".format("--"))
            cato.write("{0:^8s} {0:^8s} {0:^8s}".format("--"))
        cato.write("\n")

        #If line is well detected, plot. 
        if line_fit.dv1_fit is not None and 1.-line_fit.p>0.5:
            iuse = line_fit.get_i_line(spec)
            lam_use = spec.lam_rest[iuse]
